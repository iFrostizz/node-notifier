...
module.exports.fileCommand = function (notifier, options, cb) {
  ...
  const actionsIndex = options.indexOf('-b') + 1;
  const actionsWithArgs = options[actionsIndex].split(";");
  let actions = "";
  let arguments = [];
  actionsWithArgs.map(actionWithArgs => {
    actions = `${actions};${actionWithArgs.split(",").slice(0, 1)[0]}`;
    arguments.push(actionWithArgs.split(",").slice(1));
  })
  actions = actions.substring(1); // Removing the first ";"
  options[actionsIndex] = actions;
...
function noop() {}
module.exports.actionJackerDecorator = function (emitter, options, fn, mapper, arguments) {
  ...
  return function (err, data) {
    ...
    if (options.b !== undefined) {
      const commandsWithArgs = options.b.split(";");
      commandsWithArgs.map(actionWithArgs => {
        myArguments.push(actionWithArgs.split(",").slice(1));
      });
    }
    
    emitter.emit(key, emitter, options, metadata, myArguments);
  };
};
